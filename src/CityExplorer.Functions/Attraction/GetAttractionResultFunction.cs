...
{
    public static class GetAttractionResultFunction
    {
        [FunctionName("GetAttractionResultFunction")]
        public static async Task<WizardResult> GetResult(
            [OrchestrationTrigger] DurableOrchestrationContext context)
        {
            var criteria = context.GetInput<AttractionInputCriteria>();

            var items = await context.CallActivityAsync<IEnumerable<ResultModel>>("DataFunction", DataType.Attraction);
